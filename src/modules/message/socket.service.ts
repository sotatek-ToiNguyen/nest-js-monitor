...
export class SocketService {
  ...
  private configureSocket() {
    this.io.on(
      ...
      async (
        socket: Socket & { user: undefined | { id: number; account: string; name: string } },
      ) => {
        ...
        socket.on(
          ...
          ({ roomUid, userId }: { roomUid: string; userId: number }) => {
            console.log(1111);
            this.io.to(roomUid).emit(SOCKET_EVENT.SEEN_MESSAGE, { userId, timeSeen: new Date() });
          },
        );
        ...
        socket.on(SOCKET_EVENT.JOIN_ROOM, async ({ roomId }: { roomId: number }) => {
